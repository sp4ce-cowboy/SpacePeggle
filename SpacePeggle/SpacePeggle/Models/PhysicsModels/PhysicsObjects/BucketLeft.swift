...
final class BucketLeft: PhysicsObject {
    ...
    static let DEFAULT_CENTER = Vector(x: Constants.UI_SCREEN_WIDTH.half
                                       ...
                                       y: Constants.UI_SCREEN_HEIGHT - DEFAULT_HEIGHT.oneHalf)
    ...
    var shape: UniversalShape = BucketLeft.DEFAULT_SHAPE
    var isSubjectToGravity = false

    init(shape: UniversalShape = BucketLeft.DEFAULT_SHAPE) {
        self.shape = shape
        self.centerPosition = BucketLeft.DEFAULT_CENTER
    }
